...
int getClosestDebris()
{
    ...
    for (int debrisIndex = 3; debrisIndex < 14; ++debrisIndex)
    {
        ...
        {                                    // If the distance is less than the current min distance
...
int getSecondClosestDebris()
{
    ...
    for (int debrisIndex = 3; debrisIndex < 14; ++debrisIndex)
    {
        ...
        {                                          // If the distance is less than the second min distance
...
int getClosestWall()
{                                      // 0 for top wall, 1 for right wall, 2 for bottom wall, 3 for left wall
    float currentLocation[2] = {0.0f}; // [xcoord, ycoord]
    currentLocation[0] = game.GetRobotPositionX();
    currentLocation[1] = game.GetRobotPositionY(); // get the current location of the robot
    ...
    { // Outer loop for rows
        if (distances[wallIndex] < minDistance)
        {                                       // if the distance is less than the current min distance
...
{ // get the distance to the closest wall using the getClosestWall func
...
bool allSmallDebrisOut()
{ // the small debris are indexes 3 - 14, check if all the small debris are outside of the
    return true;
}
...
// NOTE: We dont have access to map and list types, we have
...
int (*getKeyDebris(int id))[4]
{
    ...
    for (int debris = 0; debris < 15; debris++)
    {
        ...
        int direction = inCollisionRange(id, debris);
        if (direction != -1)
        {
            neighbors[direction] = debris;
        }
    }
    return &neighbors;
}
