...
{
    int m;
    if (linePos1[0] - linePos2[0] == 0) { 
        // since they're floats this shouldn't *really* occur, but incase of sinister testcase!
        m = (linePos1[1] - linePos2[1]) / 0.01;
    } else {
        m = (linePos1[1] - linePos2[1]) / (linePos1[0] - linePos2[0]);
    }
...
int* getBestPermutation() {
    const int permutations[24][4] = {
        {0, 1, 2, 14}, {0, 1, 14, 2}, {0, 2, 1, 14}, {0, 2, 14, 1}, {0, 14, 1, 2}, {0, 14, 2, 1},
        {1, 0, 2, 14}, {1, 0, 14, 2}, {1, 2, 0, 14}, {1, 2, 14, 0}, {1, 14, 0, 2}, {1, 14, 2, 0},
        {2, 0, 1, 14}, {2, 0, 14, 1}, {2, 1, 0, 14}, {2, 1, 14, 0}, {2, 14, 0, 1}, {2, 14, 1, 0},
        {14, 0, 1, 2}, {14, 0, 2, 1}, {14, 1, 0, 2}, {14, 1, 2, 0}, {14, 2, 0, 1}, {14, 2, 1, 0}
    };
    
    float highestComputedScore = -1000.0f; 
    static int bestArr[4];
    for (int i = 0; i < 24; ++i) {
        float score = computeScore(permutations[i]);
        if (score > highestComputedScore) {
            highestComputedScore = score; 
            for (int j = 0; j < 4; ++j) {
                bestArr[j] = permutations[i][j]; 
            }
        }
    }
...
int nextDebris(const int* permutation) {
    for (int i = 0; i < 4; i++) {
        int debris = permutation[i]; 
        if (droppedOffDebris[debris]) { continue; }
        return debris; 
    }
}
...
//get the current location of the robot
    ...
    droppedOffDebris[debrisIndex] = true; 
}

void removeDebrisFar(int debrisIndex, int direction, bool specialObject) {
//get the current location of the robot
    ...
    float farPoint = (specialObject && debrisIndex > 2) ? 0.44 : 0.49; 

    if (direction == 0) {
        destinationPoint[0] = debrisLocation[0];
        destinationPoint[1] = farPoint;
    } else if (direction == 1) {
        destinationPoint[0] = farPoint;
        destinationPoint[1] = debrisLocation[1];
    } else if (direction == 2) {
        destinationPoint[0] = debrisLocation[0];
        destinationPoint[1] = -farPoint;
...
void removeDebrisWithOffsetFar(int debrisIndex, int direction, float xoffset, float yOffset, bool specialObject) {
    float (*debrisArray)[2] = getDebrisLocations();
//get the current location of the robot
...
void removeKeyDebris(int debrisIndex) {
    ...
    bool specialObject = (debrisIndex == 14);
    ...
    float closestWallDistance = getDistanceToWall(minKeyDebrisDirection, debrisLocation[0], debrisLocation[1]);
    ...
    if (directionTie) {
        ...
        { // iterate through and get all the directions that are tied, skip over the ones that have more key debris
            ...
            DEBUG(("DISTNAFETOWALL %f %f", distanceToWall, closestWallDistance));
    ...
    if (minKeyDebrisCount == 1) { // if ther is one key debris, take it and move it out
        ...
        removeDebrisFar(keyDebrisIndex, minKeyDebrisDirection, specialObject); 
    ...
    if (minKeyDebrisCount == 2) { // if ther is one key debris, take it and move it out
        ...
        float xOffset = (minKeyDebrisDirection == 1 || minKeyDebrisDirection == 3) ? 0.0f : offset;
        DEBUG(("%f", game.GetBattery()));
        removeDebrisWithOffsetFar(firstKeyDebrisIndex, minKeyDebrisDirection, xOffset, yOffset, specialObject); // this moves the astrobee to the loc outside the imaging zone that is calculated with the removebdeirs wih offset fiunction
        DEBUG(("%f", game.GetBattery()));
        // move the debris directly out since there are no key debris
        ...
        removeDebrisWithOffsetFar(secondKeyDebrisIndex, minKeyDebrisDirection, -xOffset, -yOffset, specialObject); 
...
float getBatteryCost(float* start, float* end, bool carrying, int objID) {
    float distance = getDistance(start, end);
    float mass = carrying ? game.GetObjectMass(objID) : 0;
    return distance * (12 + mass);
}

int getBestRemovalDirection(int debrisIndex) {
    int (*keyDebris)[15] = getKeyDebris(debrisIndex);
    int minKeyDebrisCount = 15;
    int minKeyDebrisDirection = -1;
    bool directionTie = false;
    ...
    if (directionTie) {
        float closestWallDistance = getDistanceToWall(minKeyDebrisDirection, debrisLocation[0], debrisLocation[1]);
        for (int direction = 0; direction < 4; ++direction) {
            if (numberOfKeyDebris[direction] != minKeyDebrisCount) continue;
            float distanceToWall = getDistanceToWall(direction, debrisLocation[0], debrisLocation[1]);
            if (distanceToWall < closestWallDistance) {
                closestWallDistance = distanceToWall;
                minKeyDebrisDirection = direction;
    ...
    return minKeyDebrisDirection;
}

double computeScore(const int *actions) {
    double battery = 100;
    double imageQualityScore = -145;
    
    float* robotPos = getCurrentLocation();
    float homePos[2] = {0.0f, 0.0f};
    
    for (int i = 0; i < 4; ++i) {
        int prioritizedDebris = actions[i]; 
        int removalDirection = getBestRemovalDirection(prioritizedDebris);
        int keyDebrisID = getKeyDebris(prioritizedDebris)[0][removalDirection];
        
        float batteryCost = estimateFullDebrisRemovalCost(prioritizedDebris, keyDebrisID, removalDirection, robotPos);
        if (battery - batteryCost < 30) {
            break; // basically doesn't consider the last priority debris removal if the cost is too high
        }
        battery -= batteryCost;
        
        if (keyDebrisID != -1) {
            imageQualityScore += 15; // points for key debris removal
        } 
        
        imageQualityScore += (prioritizedDebris == 14) ? 70 : 80; 

        robotPos = getDebrisDestinationCoords(prioritizedDebris, removalDirection); 
        
        battery += getBatteryCost(robotPos, homePos, false, -1); // this is so scuffed but estimateFullDebrisRemovalCost includes returning back home, which we won't need to consider until the very end
    }
    
    battery -= getBatteryCost(robotPos, homePos, false, -1); // now the battery subtracts by home position
    
    double batteryScore = battery;
    double finalScore = batteryScore + (1.2 * imageQualityScore);

    return finalScore;
}

float estimateFullDebrisRemovalCost(int prioritizedDebrisID, int keyDebrisID, int removalDirection, float* robotPos) {
    float* debrisPos = getDebrisLocations()[prioritizedDebrisID];
    float homePos[2] = {0.0f, 0.0f};
    float totalCost = 0;

    if (keyDebrisID != -1) {
        float* keyDebrisPos = getDebrisLocations()[keyDebrisID];
        totalCost += getBatteryCost(robotPos, keyDebrisPos, false, keyDebrisID);
        totalCost += getBatteryCost(keyDebrisPos, getDebrisDestinationCoords(keyDebrisID, removalDirection), true, keyDebrisID);
    }

    totalCost += getBatteryCost(robotPos, debrisPos, false, prioritizedDebrisID);
    totalCost += getBatteryCost(debrisPos, getDebrisDestinationCoords(prioritizedDebrisID, removalDirection), true, prioritizedDebrisID);
    totalCost += getBatteryCost(debrisPos, homePos, false, -1);

    return totalCost;
}

int* permutation; // comment this oit if it doesnt work 

//Declare any variables shared between functions here
void init() {
    // permutation = getBestPermutation(); 
    ...
    DEBUG(("predicted score %f", computeScore(array))); 
}

void loop() {
    
    while (game.GetTime() <= 180) {
        int prioritizedDebris = nextDebris(permutation); // instead of nextdebris permutation, remove that woith getClosestPrioritizedDebris (old approach),. then remove all mentions of permutation
        if (prioritizedDebris == -1) {
            game.MoveToHome(); 
            game.EndGame();
        }
        int removalDirection = getBestRemovalDirection(prioritizedDebris);
        int keyDebris = getKeyDebris(prioritizedDebris)[0][removalDirection];

        float estimatedCost = estimateFullDebrisRemovalCost(prioritizedDebris, keyDebris, removalDirection, getCurrentLocation());

        if (game.GetBattery() - estimatedCost < 37) {
            DEBUG(("Stopping early to preserve battery! Estimated cost: %f", estimatedCost));
            int array[] = {0, 14, 2, 1};
            DEBUG(("predicted score %f", computeScore(array) + (180 - game.GetTime())));
        ...
        removeKeyDebris(prioritizedDebris);
    }
    game.MoveToHome();
    game.EndGame();
}