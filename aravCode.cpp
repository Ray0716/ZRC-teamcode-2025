...
float estimateFullDebrisRemovalCost(int prioritizedDebrisID, int keyDebrisID, int removalDirection) {
...
void loop() {
    while (game.GetTime() <= 180) {
        if (game.GetBattery() <= 50) {
            int prioritizedDebris = getClosestPrioritizedDebris();
            int keyDebris = getKeyDebris(prioritizedDebris)[0][0];
            int removalDirection = getBestRemovalDirection(prioritizedDebris);

            float estimatedCost = estimateFullDebrisRemovalCost(prioritizedDebris, keyDebris, removalDirection);

            if (game.GetBattery() - estimatedCost < 30) {
